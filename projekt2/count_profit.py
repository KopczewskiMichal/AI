...
BUY_DAY = TODAY - timedelta(days=5) # Eliminujemy przypadek błędu w poniedziałek, ponieważ w niedzielę giełda jest zamknięta
...
tested =  ['TT', 'JNJ', 'VST', 'NVDA', 'VST', 'LDOS', 'NVDA', 'NTAP', 'QCOM', 'NRG']
best_tickers = ['PGR', 'FANG', 'VST', 'NVDA', 'PGR', 'CL', 'ELV', 'VST', 'TT', 'LDOS']
most_profitable_tickers = ['VST', 'SMCI', 'NVDA', 'NRG', 'CEG', 'DECK', 'MU', 'WDC', 'LLY', 'GE']
...
if __name__ == '__main__':
    print(f"testowany przypadek {round(count_profit(tested) * INVESTED, 2)}$")
    print(f"Najoptymalniejszy algorytm zarobił {round(count_profit(best_tickers) * INVESTED, 2)}$")
    print(f"Najbardziej zyskowny (ale bardzo ryzykowny algorytm zarobił {round(count_profit(most_profitable_tickers) * INVESTED, 2)}$")
